...
- **Log File Locations:** By default, LogFlow resolves logs to `./logs`. However, in this workspace, the global configuration at `~/.config/logflow/config.yaml` redirects all logs to `~/logs` for centralized persistence. Always check `~/logs` when debugging local execution.
